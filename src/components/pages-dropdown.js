...
export const PagesDropdown = () => (
  ...
  >
    ...
    >
      ...
      >
        <Box sx={{ p: 3 }}>
          <Typography color="textPrimary" sx={{ mb: 1.25 }} variant="h4">
            Pages
          </Typography>
          <Typography color="textSecondary" variant="body1">
            Carpatin - Admin Dashboard includes 6 unique user-flows with a total
            of 30 screens. We think the value is in the quality and not
            quantity.
          </Typography>
        </Box>
        <Box sx={{ flexGrow: 1 }} />
        <img alt="" src="/static/feature-auth.png" />
      </Box>
      <Box
        sx={{
          flexGrow: 1,
      ...
      >
        <Grid container spacing={3}>
          {[sections.slice(0, 2), sections.slice(2, 4), sections.slice(4)].map(
            (sectionArr, index) => (
              <Grid
                item
              ...
              >
                {sectionArr.map((section) => (
                  <PagesDropdownSection key={section.title} section={section} />
                ))}
              </Grid>
            ),
          )}
