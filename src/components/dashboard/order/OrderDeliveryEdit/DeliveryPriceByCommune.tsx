...
function DeliveryPriceByCommune({ wilayaId, companyId, handleChange, officeId }: any) {
                ...
                >
                    {!error && !isLoading && OfficesData && <>
                        {OfficesData?.map((item: any) => (
                            ...
                            >
                                <Typography className="title-price">{item.delivery_price} DZD</Typography>
                                <Typography className="meta-loc">
                                    <LocationCity /> {item.commune?.name}, {item.wilaya?.name}
                                </Typography>
                                <Typography className="meta-text">{item.address.slice(0, 10)}</Typography>
                            </ZimouRadio>
                        ))}
                    </>}
                </div>
