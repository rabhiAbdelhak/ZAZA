...
import EnterVoucherValidateDialog from './enter-voucher-validation-dialog'
// import EnterVoucherValidateDialog from './purchase-order-validate-dialog'

type EnterVoucherTableProps = {
  selected?: EnterVoucher
  onSelect?: (enterVoucher: EnterVoucher) => void
  data?: EnterVoucher[]
  isLoading?: boolean
  compact?: boolean
  error?: any
  filter?: EnterVoucherFilter
  onFilter?: (filter: EnterVoucherFilter) => void
}

function EnterVoucherTable({
  data = [],
  selected,
  isLoading = false,
  compact = false,
  ...
  const [dialogItem, setDialogItem] = useState<EnterVoucher>()

  const openValidateDialog =
    (item: EnterVoucher) => (event: MouseEvent<HTMLButtonElement>) => {
...
export default EnterVoucherTable
