...
import EnterVoucherProducts from './enter-voucher-products'
import WarehouseSelect from '../../warhouse/warehouse-select'
import VoucherPrintModal from '../exit-voucher/PDFVoucher/VoucherPrintModal'

type EnterVoucherFormProps = {
  initialValues?: Partial<EnterVoucher>
  isEditMode?: boolean
  isDraftMode?: boolean
  fromId?: string
  onSubmit: (data: Partial<EnterVoucherFormData>) => void
}
function EnterVoucherForm({
  initialValues,
  isEditMode = false,
  isDraftMode = true,
  fromId,
  onSubmit,
}: EnterVoucherFormProps) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const data: EnterVoucherFormData = {
  ...
  return (
    <Box id={fromId} component="form" onSubmit={formik.handleSubmit}>
      ...
      <VoucherPrintModal
        ...
        data={print}
        onClose={closePrint}
        typeOfDocument={'EntryVouchers'}
      />
    </Box>
  )
}

export default EnterVoucherForm
