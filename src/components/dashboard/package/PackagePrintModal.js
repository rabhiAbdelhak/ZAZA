...
const PackagePrintModal = ({ data = [], open = false, onClose }) => {
  const { user } = useAuth()
    ...
    <Dialog fullWidth maxWidth="lg" open={open}>
      ...
      {canRender && (
        <PDFViewer style={{ height: 'calc(100vh - 96px)', width: '100%' }}>
          <PDFPackage user={user} data={data} isA6Format={isA6Format} />
        </PDFViewer>
      )}

      <Container maxWidth="xs" fullWidth sx={{ py: 2 }}>
        <InputLabel
          sx={{
            color: 'text.primary',
            fontSize: 14,
...
export default PackagePrintModal
